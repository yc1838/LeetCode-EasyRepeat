(function (root, factory) {
    ...
    const DICTIONARY = {
        en: {
            ...
            content_step_captured_failed: 'Captured failed submission',
            content_step_analyzing_error: 'Analyzing error pattern',
            content_step_searching_kb: 'Searching knowledge base',
            content_step_found_solution: 'Found existing solution',
            content_step_safe_observer: 'Safe Observer verification',
            content_step_generate_tests: 'Generating edge-case tests',
            content_step_generate_fix_attempt: 'Generating fix (attempt {attempt}/{total})',
            content_step_execute_sandbox_attempt: 'Executing in sandbox (attempt {attempt}/{total})',
            content_step_verified_success: 'Safe Observer verified',
            content_step_verified_failed: 'Safe Observer failed',
            content_step_consulting_model: 'Consulting AI model',
            content_step_analysis_complete: 'Analysis complete',
            content_step_analysis_failed: 'Analysis failed',
            content_ai_analysis_heading: 'AI Analysis',
            content_mistake_label: 'Mistake',
            llm_analysis_title: 'Mistake Analysis',
            llm_submission_status: 'Submission status',
            llm_why_wrong: 'Why it failed',
            llm_correct_approach: 'Correct approach',
            llm_correct_code: 'Correct example',
            llm_missing_to_solution: "What's still missing",
            llm_gap_summary: 'Distance from a correct solution',
            llm_next_step: 'Recommended next step',
            llm_skill: 'Skill',
            llm_state_attempted: 'Attempted, but contains an error',
            llm_state_incomplete: 'Implementation is substantially incomplete',
            llm_state_empty: 'No effective solution was submitted',
            llm_state_unavailable: 'Submitted code could not be captured',
            llm_capture_failed_title: 'Unable to analyze the submitted code',
            llm_capture_failed_hint: 'The extension could not read the editor content, so it cannot reliably identify why this submission failed.',
            llm_capture_failed_action: 'Keep the code editor visible, refresh the LeetCode page, and submit again.',
            llm_empty_submission_hint: 'There is no solution logic to analyze yet. Start by writing the core algorithm and the required return value.',
            llm_incomplete_submission_hint: 'This implementation is still missing substantial logic. Complete the core algorithm, state transitions, and return path before focusing on a local bug.',
            llm_partial_capture_notice: 'The editor snapshot may be incomplete, so this analysis is limited to the code that was captured.',
            llm_no_missing_items: 'No additional missing step was returned.',
            llm_field_unavailable: 'The model did not provide this part of the analysis.',
            llm_recurring_title: 'Recurring mistake detected',
            llm_previous_advice: 'Previous advice',
        ...
        zh: {
            ...
            content_step_captured_failed: '已记录失败提交',
            content_step_analyzing_error: '分析错误模式',
            content_step_searching_kb: '正在搜索知识库',
            content_step_found_solution: '找到现有解决方案',
            content_step_safe_observer: 'Safe Observer 验证',
            content_step_generate_tests: '生成边界测试',
            content_step_generate_fix_attempt: '生成修复（第 {attempt}/{total} 次）',
            content_step_execute_sandbox_attempt: '沙箱执行（第 {attempt}/{total} 次）',
            content_step_verified_success: 'Safe Observer 验证通过',
            content_step_verified_failed: 'Safe Observer 验证失败',
            content_step_consulting_model: '正在咨询 AI 模型',
            content_step_analysis_complete: '分析完成',
            content_step_analysis_failed: '分析失败',
            content_ai_analysis_heading: 'AI 错误分析',
            content_mistake_label: '错误类型',
            llm_analysis_title: '错误分析',
            llm_submission_status: '提交状态',
            llm_why_wrong: '为什么错',
            llm_correct_approach: '正确思路',
            llm_correct_code: '正确写法示例',
            llm_missing_to_solution: '距离正确答案还缺什么',
            llm_gap_summary: '距离正确答案的差距',
            llm_next_step: '建议下一步',
            llm_skill: '需要补强的技能',
            llm_state_attempted: '已作答，但代码中仍有错误',
            llm_state_incomplete: '实现缺失较多',
            llm_state_empty: '未提交有效解答',
            llm_state_unavailable: '未能读取本次提交的代码',
            llm_capture_failed_title: '暂时无法分析本次代码',
            llm_capture_failed_hint: '扩展没有成功读取编辑器内容，因此无法可靠判断这次提交为什么出错。',
            llm_capture_failed_action: '请保持代码编辑器可见，刷新 LeetCode 页面后重新提交。',
            llm_empty_submission_hint: '目前还没有可分析的解题逻辑。请先写出核心算法和题目要求的返回值。',
            llm_incomplete_submission_hint: '当前实现仍缺少较多关键逻辑。请先补齐核心算法、状态变化和返回路径，再定位局部错误。',
            llm_partial_capture_notice: '编辑器快照可能不完整，本次分析仅依据已读取到的代码。',
            llm_no_missing_items: '模型没有返回额外的缺失项。',
            llm_field_unavailable: '模型没有提供这一部分分析。',
            llm_recurring_title: '检测到重复错误',
            llm_previous_advice: '之前的建议',
